...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (event.status !== 'upcoming') {
      return NextResponse.json({ error: 'Cannot join completed events' }, { status: 400 });
    }

    if (new Date(event.event_date) < new Date()) {
      return NextResponse.json({ error: 'Cannot join past events' }, { status: 400 });
    }
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (participantError || !participant) {
      return NextResponse.json({ error: 'Not a participant of this event' }, { status: 404 });
    }
